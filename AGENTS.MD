...
- Output is markdown in `content` (no `structuredContent`). Post content is wrapped in `<piazza_post>` tags and the server instructions tell the model it's untrusted class-member text; per-section and per-response size caps keep outputs bounded.
