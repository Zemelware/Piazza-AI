...
- `src/tools.js`: Tool definitions, all prefixed `piazza_`. Designed around workflows rather than API endpoints: `piazza_find_posts` (keyword/folder/filter, combinable, returns summaries) then `piazza_read_posts` (batch of up to 10, `concise`/`full` detail). Also `piazza_list_classes` (includes folders), `piazza_get_class_info`, `piazza_login`. Read tools are annotated `readOnlyHint: true`. Write tools (`piazza_create_post`, `piazza_add_followup`) are always registered, annotated as non-read-only, default to anonymous, and each call must be approved by the user through an MCP elicitation prompt.
- Output is text in `content` (no `structuredContent`). Posts from `piazza_read_posts` use XML elements for structure (`<piazza_post>` with attributes, `<subject>`, `<question>`/`<note>`, `<instructor_answer>`, `<student_answer>`, `<followup>`, `<reply>`) and markdown for the text inside. Those tag names are neutralized in post text so class-member content can't fake structure; server instructions say post content is untrusted. Per-section and per-response size caps keep outputs bounded.
